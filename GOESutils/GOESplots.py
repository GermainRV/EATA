...
pol_lima = gdf_peru_land[gdf_peru_land['name'] == "Lima"].geometry.iloc[0]
pol_lima_prov = gdf_peru_land[gdf_peru_land['name'] == "Lima Province"].geometry.iloc[0]
pol_callao = gdf_peru_land[gdf_peru_land['name'] == "Callao"].geometry.iloc[0]
ind_lima = gdf_peru_land[gdf_peru_land['name'] == "Lima"].index[0]
gdf_peru_land.at[ind_lima,'geometry'] = pol_lima.union(pol_callao)#.union(pol_lima_prov)
gdf_peru_land = gdf_peru_land[~gdf_peru_land['name'].isin(["Callao"])]#"Lima Province", 
...
def definingColormaps(disp=True):
    ...
    product_colormaps = {
        ...
        'ABI-L2-RRQPEF': dict(cmap=cmaps.ncview_default, min = 0, max = 100),#cmaps.deep,
...
def GeoColorPlot(RGBdata, GeoColorParams, toSave=False, toDisplay=True, department=False):
    ImageTime = GeoColorParams["ImageTime"]
...
def ProductPlot(data_re, product, axGeo, ProductParams, toSave=False, title="Peru"):
    """
    ...
    if isACM or isACTP:
        ...
        cbar = plt.colorbar(im,ax=axProd, orientation='horizontal', extend="both", shrink=0.7, pad=0.01)
        units_latex = re.sub(r'(\w)(-)(\d)', r'\1^{-\3}', data_re.units)
    ...
    if toSave:
        ...
        axProd.figure.savefig(ProductParams["ImageFullPath"],dpi=600,bbox_inches='tight')
    plt.close()
